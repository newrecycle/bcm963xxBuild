...
function cujo.jobs.spawn(f, ...) cospawn.catch(errhandler, f, ...) end
...
function cujo.jobs.exec(cmd, args)
	cujo.log:jobs("starting '", cmd, ' ', args)
	local proc, err = process.create{
		execfile = assert(cmd, 'missing command'),
		arguments = args,
	}
	if not proc then
		cujo.log:error("failed to run '", cmd, ' ', args, "' (", err, ')')
		return nil, err
	end
	...
	repeat
		...
		if exitval == 0 then
			cujo.log:jobs("command '", cmd, "' ", args, ' executed successfully')
			return true
		end
		if exitval then
			cujo.log:warn("command '", cmd, "' ", args, ' exit with error ', exitval)
			return nil, exitval
		end
		if err ~= 'unfulfilled' then
			cujo.log:error("error running command '", cmd, "' ", args, ' (', err, ')')
			return nil, err
		end
	until os.time() >= finish
	cujo.log:warn("command '", cmd, "' ", args, ' is taking too long')
	return nil, 'timeout'
end
