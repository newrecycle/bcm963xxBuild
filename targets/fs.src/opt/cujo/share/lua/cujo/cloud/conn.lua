...
function stompcb.established(conn)
	...
	cujo.jobs.spawn(broker_timeout_handler, conn)
...
function stompcb.connected(conn)
	...
	cujo.jobs.spawn(client_timeout_handler, conn)
...
function stompcb.message(conn, headers, body)
	...
	inmsgs:enqueue{channel, json.decode(body, 'nil')}
end
...
function connect()
	local url, err = cloudauth()
	if not url then
		cujo.log:error('STOMP failed to get authetication url (', err, ')')
...
cujo.jobs.spawn(function ()
	while true do
		...
		local ok, err = xpcall(publishers[channel],
			debug.traceback, channel, body)
		if not ok then
