...
#if defined(GPL_CODE_A_DHCP_OPTION60) && defined(GPL_CODE_B_R3000)
	...
	while(1) { /* loop until universe collapses */
		...
		if (FD_ISSET(msg_fd, &rfds)) {
            ...
            case CMS_MSG_DHCPD_RELOAD:
					...
					read_leases(server_config.lease_file);
               break;
#if defined(GPL_CODE) && !defined(GPL_CODE_B_DHCP_LEASE)
            ...
            case CMS_MSG_GET_LEASE_TIME_REMAINING:
               if (msg->dataLength == sizeof(GetLeaseTimeRemainingMsgBody)) {
					   ...
					   if (cur_iface != NULL) {
						   ...
						   if (lease != NULL) {
#if defined (GPL_CODE_T) || defined(GPL_CODE_B)
								//static leases
...
#if defined(GPL_CODE_C)
                                          if (
#if defined(GPL_A_DHCP_SPECCHAR)
...
#ifdef GPL_CODE_C //add william 2012-4-25
