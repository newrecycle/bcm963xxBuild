...
DhcpcStateChangedMsgBody dhcpc_opt = 
    ...
    0,
#endif
#if defined(GPL_CODE_A_DHCP_WAN_LEASETIME)
    0,
#endif
...
#if defined(GPL_A_SUPPORT_6RD) && defined(GPL_A_CONTROL_LAYER)
extern void AEI_sendEventMessageWith6RD(UBOOL8 assigned, DhcpcStateChangedMsgBody *options);
#endif
struct aei_wan_info_t current_wan;
...
void AEI_dhcpcSetWan(DhcpcStateChangedMsgBody *options)
{
    char cmd[256] = {0};

    if (options && client_config.interface)
    {
        if (options->ip && options->mask &&
            (strcmp(current_wan.ip, options->ip) ||
            strcmp(current_wan.mask, options->mask)))
        {
            snprintf(cmd, sizeof(cmd)-1, "ifconfig %s %s netmask %s up", client_config.interface, options->ip, options->mask);
            system(cmd);
            strncpy(current_wan.ip, options->ip, sizeof(current_wan.ip)-1);
            strncpy(current_wan.mask, options->mask, sizeof(current_wan.mask)-1);
        }

        if (options->gateway &&
            strcmp(current_wan.gateway, options->gateway))
        {
            snprintf(cmd, sizeof(cmd)-1, "route del default dev %s", client_config.interface);
            system(cmd);
            snprintf(cmd, sizeof(cmd)-1, "route add default gw %s dev %s", options->gateway, client_config.interface);
            system(cmd);
            strncpy(current_wan.gateway, options->gateway, sizeof(current_wan.gateway)-1);
        }

        if (options->nameserver)
        {
            char nameserver[64] = {0};
            strncpy(nameserver, options->nameserver,sizeof(nameserver)-1);
            if (strcmp(current_wan.nameserver, nameserver))
            {
                char nameserver1[32] = {0};
                char nameserver2[32] = {0};
                char* ptok=NULL;
                ptok=strtok(nameserver, ",");
                if(ptok!=NULL)
                {
                    snprintf(nameserver1, sizeof(nameserver1)-1, "%s", ptok);
                    ptok = strtok(NULL, ",");
                    if(ptok!=NULL)
                        snprintf(nameserver2, sizeof(nameserver2)-1, "%s", ptok);
                }
                AEI_updateNameServer(nameserver1, nameserver2);
                strncpy(current_wan.nameserver, options->nameserver, sizeof(current_wan.nameserver)-1);
            }
        }

        if (!AEI_checkIfMasqueradeRuleExist(client_config.interface))
        {
            if (AEI_insertModuleIptablesMasquerade() != -1)
            {
                snprintf(cmd, sizeof(cmd)-1, "iptables -t nat -A POSTROUTING -o %s -s 0/0 -j MASQUERADE 2>/dev/null", client_config.interface);
                system(cmd);
            }
        }

        if (!AEI_checkIfIpv4ForwardEnabled())
        {
            snprintf(cmd, sizeof(cmd)-1, "echo 1 > /proc/sys/net/ipv4/ip_forward");
            system(cmd);
        }

        if (AEI_checkIfDnsInfoConfEmpty() && options->nameserver)
            AEI_updateDnsProxy(client_config.interface, current_wan.nameserver);
    }
}
#else
//brcm
...
static char **fill_envp(struct dhcpMessage *packet)
{
	...
	envp[j] = NULL;
       ...
       AEI_dhcpcSetWan(&dhcpc_opt); // To keep WAN still working when BRCM CMS not working.
       AEI_sendEventMessage(TRUE,FALSE,&dhcpc_opt);
#else	     
   sendEventMessage(TRUE, FALSE, &dhcpc_opt);
#endif	   
#if defined(GPL_A_CONTROL_LAYER) && defined(GPL_A_SUPPORT_6RD)
...
#if defined(GPL_CODE_A_DHCP_WAN_LEASETIME) //add william 2011-12-5
