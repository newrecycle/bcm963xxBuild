...
#if defined(DMP_X_BROADCOM_COM_IPV6_1) || defined(GPL_A_CONTROL_LAYER) /*added by libby 2012-04-25*/
...
#define MAX_NUM_OF_DENIEDSITES      10
#define MAX_LENGTH_OF_DENIEDSITES      20
typedef struct {
    char dns_name[MAX_LENGTH_OF_DENIEDSITES]; //The name of html, cgi, cmd and so on
}VUL_DNS_ENTRY;
VUL_DNS_ENTRY          denied_sites[MAX_NUM_OF_DENIEDSITES];
void AEI_init_deniedSites ()
{
    int i = 0;
  for (i = 0;i< MAX_NUM_OF_DENIEDSITES;i++)
  {
    memset(&denied_sites[i],0,sizeof(VUL_DNS_ENTRY));
  }
}

void AEI_get_deniedSites (const char *lst)
{
    char *buf = NULL;
    char *site = NULL;
    char *last = NULL;
    ...
    if(!lst)
    {
        cmsLog_error("dns_deny_sites do not exist,clear dns deny sites");
        AEI_init_deniedSites ();
        return;
    }

    if ((buf = cmsMem_strdup(lst)) == NULL)
        return;

    AEI_init_deniedSites ();
    for (site = strtok_r(buf, "|", &last); site != NULL; site = strtok_r(NULL, "|", &last)) {
        memset(&denied_sites[i],0,sizeof(VUL_DNS_ENTRY));
         sprintf(denied_sites[i].dns_name,"%s",site);
         cmsLog_notice("denied idx=%d,dns=%s",i,site);
         i++;
         if(i == MAX_NUM_OF_DENIEDSITES)
             break;
    }
...
int match_site_entry( const char* pattern, int patternlen, const char* string )
{
    const char* p;
    ...
    for ( p = pattern; p - pattern < patternlen; ++p, ++string )
    {
        ...
        if ( *p == '*' )
        {
            ...
            for ( ; i >= 0; --i )
                if ( match_site_entry( p, pl, &(string[i]) ) )
                    return 1;
            return 0;
        }
        if ( *p != *string )
...
#if defined(GPL_A_FRONTIER_WIZARD)
...
#if defined(GPL_CODE)
void AEI_utlWritePidToFile(const char *path)
{
    FILE *fp = NULL;
    char buf[16] = {0};
    ...
    {
        snprintf(buf, sizeof(buf)-1, "%u", (unsigned int)getpid());
        fputs(buf, fp);
        fclose(fp);
    }
}
...
void dns_handle_new_query(dns_request_t *m)
{
   ...
   int denied_check = 0;
      int i = 0;
      char tmp_site[MAX_LENGTH_OF_DENIEDSITES] ={0};
      cmsLog_notice("name=%s>>>",m->cname);
      for (i = 0;i< MAX_NUM_OF_DENIEDSITES;i++)
      {
          if(strlen(denied_sites[i].dns_name) == 0)
              break;

          cmsLog_notice("dns_name[%d]=%s>>>",i,denied_sites[i].dns_name);
          sprintf(tmp_site,"%s",denied_sites[i].dns_name);
          if(match_site_entry( tmp_site, strlen(tmp_site), m->cname ))
          {
              cmsLog_error("++++++deny query %s",m->cname);
              denied_check = 1;
              retval = 0;
          }
   ...
   { /* IPv6 todo */
#if defined(GPL_A_CONTROL_LAYER)
		/*modify by libby support IPV6*/
#ifdef GPL_CODE_B_DNS_CACHE
		char name_app[128] = {0};
		snprintf( name_app, sizeof(name_app)-1, "[%s]", m->cname );
		retval = cache_lookup_name( name_app, m->ip );
...
#endif
#ifdef GPL_A_AUTODNS_VULNERABLE
    case CMS_MSG_DNSPROXY_UPDATE_DENIED_SITES:
    {
        DeniedSitesMsgBody *body = (DeniedSitesMsgBody *)(msg+1);
        cmsLog_notice("got dns sites message");
        if(msg->wordData == 1)
        AEI_get_deniedSites (body->deniedSiteList);
        else
        AEI_get_deniedSites (NULL);
    }
        break;
#endif
...
#if defined(DMP_X_BROADCOM_COM_IPV6_1) || defined(GPL_A_CONTROL_LAYER)
	char *Ip=NULL;		
#endif    
...
#define DNS_PROXY_PID_FILE "/var/run/dnsproxy.pid"
 AEI_utlWritePidToFile(DNS_PROXY_PID_FILE);
 signal(SIGUSR2, dns_mapping_conifg_init);
#endif
