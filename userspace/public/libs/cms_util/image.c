...
CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, UINT32 opts, int * noReboot)
{
    ...
    if (format == CMS_IMAGE_FORMAT_FLASH)
    {
        ...
        if ((ret = getFlashInfo(&flags)) == CMSRET_SUCCESS)
        {
           ...
           { // NAND flash device
#ifdef GPL_A_CENTURYLINK_UPGRADE_ACTIVE_FW
...
#if !defined(GPL_CODE_C)
#if defined(GPL_CODE_B_USB_LOAD_CONFIGFILE)
         char configinfo[2] = {0};
...
CmsRet AEI_verifyImageForBoard(const char * imagePtr, UINT32 imageLen)
{
    ...
    if(wt.wfiFlashType == WFI_NAND128_FLASH)
        offset=0x20000;
    if(wt.wfiFlashType == WFI_NAND16_FLASH)
        offset=0x4000;

    if((*(unsigned short *)imagePtr == JFFS2_MAGIC_BITMASK && *(unsigned short *)(imagePtr+2) == AEI_MAGIC_BITMASK))
    {    
        offset = 0; 
        pTag = (FILE_TAG *)imagePtr;
    }
    else if((*(unsigned short *)(imagePtr+offset) == JFFS2_MAGIC_BITMASK && *(unsigned short *)(imagePtr+offset+2) == AEI_MAGIC_BITMASK))
    {
        pTag = (FILE_TAG *)(imagePtr+offset);
    } 
...
CmsImageFormat cmsImg_validateImage(const char *imageBuf, UINT32 imageLen, void *msgHandle)
{
   ...
   cmsLog_debug("not a ModSW Linux PFP");
       ...
       devCtl_boardIoctl(0x1, 0, NULL, 0, 0, (void *)&offset);
   ...
   {
#if defined(GPL_A_CONFIG_JFFS) && defined(GPL_A_63168_CHIP)
