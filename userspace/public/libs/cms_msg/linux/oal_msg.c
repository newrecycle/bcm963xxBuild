...
int cmsMsg_blockLog(char *str)
{
    ...
    fd = open("/tmp/temp", O_CREAT|O_RDWR|O_APPEND);
    if(fd > 0){
        fstat(fd, &st);
        if(st.st_size < 1000){
            ret = write(fd, str, strlen(str));
        }else{
            cmsLog_error(" has reached the maximum!");
            ret = 0;
        }
        close(fd);
    }
...
int set_msg_socket_snd_timeout(int fd, UBOOL8 *blkState, UBOOL8 noblock, const CmsMsgHeader *hdr)
{
    ...
    if(*blkState != noblock)
    {
        ...
        if(noblock)
        {
            ret = set_socket_opt(fd, SO_SNDTIMEO, 1);
            if(!ret){
...
#if defined(GPL_CODE_B_CMS_MSG_NOBLOCK)
      set_socket_opt(handle->commFd, SO_SNDTIMEO, 1);
